...
const REGISTRY_ABI = [
  "function issue(bytes32 seriesId, address subject, string metadataCid, bytes32 contentHash)",
  "function addVersion(bytes32 seriesId, string metadataCid, bytes32 contentHash)",
...
const registry = new ethers.Contract("0xc5f035Dbe14d700bD57F9B08D92105629C9ff66b", REGISTRY_ABI, wallet);
...
export async function chainIssue({ seriesId, subject, metadataCid, contentHash }) {
  console.log(seriesId,subject,metadataCid,contentHash)
  const tx = await registry.issue(seriesId, subject, metadataCid, contentHash);
  const receipt = await tx.wait();
  return { txHash: receipt.transactionHash || receipt.hash };
}

export async function chainAddVersion({ seriesId, metadataCid, contentHash }) {
  const tx = await registry.addVersion(seriesId, metadataCid, contentHash);
  const receipt = await tx.wait();
  return { txHash: receipt.transactionHash || receipt.hash };
}
